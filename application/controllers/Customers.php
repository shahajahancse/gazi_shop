...
class Customers extends MY_Controller {
	...
	public function ajax_list()
	{
		...
		foreach ($list as $customers) {
			...
			$row[] = $customers->email;
			$row[] = (!empty($customers->sales_due) && $customers->sales_due!=0) ? $this->currency($customers->sales_due) : $this->currency(0);
			$row[] = ($customers->sales_return_due==null) ? $this->currency(0) : $this->currency($customers->sales_return_due);
